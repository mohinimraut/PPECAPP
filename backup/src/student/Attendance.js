...
async Attendacnedata() {
  ...
  console.log('student id:' + this.state.stud_id)
 ...
 console.log(JSON.stringify(response))
  ...
  if (response != 'error') {
    ...
    for (var i = 0; response.holiday.length > i; i++) {
      ...
      cl[date] = { "periods":[ { startingDay: true, endingDay: true, color: '#00bcd4' }] }
    }
    this.setState({ spinner: false })
    //console.log('1');
 ...
 for (var i = 0; this.state.dailydata.attendance.length > i; i++) {
  ...
  if (date2 == newdate) {
    ...
    if(date.getHours()=='0'){
      ...
      var intime=this.state.dailydata.attendance[i].in_time.substring(9);
      var outtime=this.state.dailydata.attendance[i].out_time.substring(9);
    }
    this.setState({ 
      intime: intime,
      outtime:outtime,
        ...
        <View style={{ flex: 1, alignContent: "center" ,backgroundColor:'#cfd8dc'}}>
          <Header style={{ backgroundColor: "white" }}>
            ...
            <View style={{ flexDirection: "row" ,justifyContent:'center',alignItems:'center'}}>
              ...
              >
                <Image
                  source={require("../../assets/Images/HomeImages/leftarroww.png")}
            ...
            <Text style={{ color: "#F57C00", fontSize: 20,fontWeight:'bold'}}>ATTENDANCE</Text>
            </View>
          </Header>
          <ImageBackground
